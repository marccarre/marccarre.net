...
2. Tampering: the modification of data persisted within the system, e.g.: [SQL injection](https://owasp.org/www-community/attacks/SQL_Injection), [MITM](https://owasp.org/www-community/attacks/Man-in-the-middle_attack), [DNS cache poisoning](https://www.cs.cornell.edu/~shmat/shmat_securecomm10.pdf).
...
4. Information Disclosure: the acquisition of data by a trust level that should not have access to it, e.g.: bad messaging, [SQL injection](https://owasp.org/www-community/attacks/SQL_Injection), [MITM](https://owasp.org/www-community/attacks/Man-in-the-middle_attack).
5. Denial of Service: preventing legitimate users from accessing the service, e.g.: [SQL injection](https://owasp.org/www-community/attacks/SQL_Injection), [DDOS](https://owasp.org/www-community/attacks/Denial_of_Service).
6. Elevation of Privilege: an attack aimed at allowing an entity of lower trust level to perform actions restricted to a higher trust level, e.g.: [SQL injection](https://owasp.org/www-community/attacks/SQL_Injection), [CSRF](https://owasp.org/www-community/attacks/csrf).
